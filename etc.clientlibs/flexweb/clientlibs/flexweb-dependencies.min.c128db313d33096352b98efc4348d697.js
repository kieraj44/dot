...
window.$CQ=_g.$;
function digitalDataManager(b){var a=this;
a.data={};
a.pageName="na";
a.channel="na";
a.isChannelMapped=false;
a.channelIndex=0;
a.site="main";
a.brand="gdc";
a.emptyPageDefault="home";
a.overrides=b;
a.getDigitalData=function(c,e){try{a._setOverrideValues();
a._initializeDigitalData();
a._parseLocation(c);
a._setPageAndChannel();
a._setAdobeCampaign(e)
}catch(d){console.error("Error loading digital data object")
}return a.data
};
a._setOverrideValues=function(){if(!a.overrides){return
}if("emptyPageDefault" in a.overrides){a.emptyPageDefault=a.overrides.emptyPageDefault
}if("brand" in a.overrides){a.brand=a.overrides.brand
}if("isChannelMapped" in a.overrides){a.isChannelMapped=a.overrides.isChannelMapped
}if("channelIndex" in a.overrides){a.channelIndex=a.overrides.channelIndex
}};
a._setAdobeCampaign=function(d){if(!d){return
}var c=d[0];
if(!c){return
}a.data.abTest.adobeTarget.campaignID=c.CampaignId;
a.data.abTest.adobeTarget.campaignName=c.CampaignName;
a.data.abTest.adobeTarget.recipeID=c.RecipeId;
a.data.abTest.adobeTarget.recipeName=c.RecipeName;
a.data.abTest.adobeTarget.offerID=c.OfferId;
a.data.abTest.adobeTarget.offerName=c.OfferName;
a.data.abTest.adobeTarget.mboxName=c.MboxName
};
a._setPageAndChannel=function(){a.data.page.pageInfo.channel=a.channel;
a.data.page.pageInfo.pageName=a.brand+":"+a.site+":"+a.channel+":"+a.pageName
};
a._mapChannel=function(){if(a.channel==null||a.channel=="undefined"||a.channel==""){a.channel="home"
}};
a._parseLocation=function(c){if(!c||!c.pathname){return
}var e=c.pathname.substring(1).toLowerCase();
var d=e.split("/");
if(d.length>0){a.pageName=d.slice(-1)[0];
a.channel=d[a.channelIndex];
if(a.isChannelMapped){a._mapChannel()
}if(a.pageName==null||a.pageName=="undefined"||a.pageName==""){a.pageName=a.emptyPageDefault
}}};
a._initializeDigitalData=function(){a.data={page:{pageInfo:{pageName:"",brand:a.brand,site:a.site,channel:"",siteRendered:"desktop"}},user:[{profile:[{profileInfo:{custId:"not_applicable",status:"logged_out"}}]}],abTest:{adobeTarget:{campaignID:"",campaignName:"",recipeID:"",recipeName:"",offerID:"",offerName:"",mboxName:""}}}
}
};